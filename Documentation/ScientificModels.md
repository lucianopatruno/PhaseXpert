...
An experimental native teqp provider is also available. It is not the default
...
| teqp | Offline C++ equation-of-state framework with native pure-CO₂ support, EOS-CG-2021 property-specific H₂/CH₄ density slices, diagnostic binary mixture Cv/Cp/speed calculations and validation-gated generic binary VLE code in PhaseXpert | Experimental in PhaseXpert; user-facing mixture scope is limited to explicitly validated density slices and a narrow corrected CO₂+CH₄ VLE/phase-classification gate with a continuous production bubble/dew segment inside validated Petropoulou temperature bounds; diagnostic mixture heat capacities and speed of sound remain hidden until independent property validation passes; transport and reference-state properties remain unavailable |
...
property availability and phase-diagram status are generated from provider
capability metadata, not separate SwiftUI-maintained scientific strings. This
guidance is a discoverability layer only; unsupported, diagnostic-only and
mathematically EOS-computable states remain unavailable unless an independent
production validation gate is encoded.
