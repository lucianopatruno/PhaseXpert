...
The follow-on global-stability milestone implemented deterministic fixed-T/P TPD minimization and a direct-composition generic TP flash without changing these accepted boundary metrics or their research-only status. Its equations, validation scope, diagnostics and limitations are recorded in `AdvancedCCSGlobalStabilityTPFlash2026-08-20.md`. Rachford–Rice was intentionally unnecessary for the selected direct phase-composition/material-balance formulation. Envelope continuation remains future research. No hidden global cache or mutable solver state is used; identical inputs follow identical multistarts.
